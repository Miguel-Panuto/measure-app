...
class ChartCpuScreen extends StatefulWidget {
  @override
  _ChartCpuScreenState createState() => _ChartCpuScreenState();
}

class _ChartCpuScreenState extends State<ChartCpuScreen> {
  LineChartBarData cpuUsage;
  bool isLoading = true;
  ...
  Future<void> initUsages(PcProvider pcProvider) async {
    ...
    final _cpuUsage = await pcProvider.getCpuUsage();

    cpuUsage = LineChartBarData(
  ...
  void attUsages(PcProvider pcProvider) {
    Timer.periodic(const Duration(seconds: 1), (timer) async {
      ...
      if (cpuUsage.spots.length > 10) {
        cpuUsage.spots.removeAt(0);
      }

      setState(() {
        cpuUsage.spots.add(FlSpot(actualTime.inSeconds.toDouble(), _cpuUsage));
      });
    });
  }
